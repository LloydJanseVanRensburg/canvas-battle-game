...
const btnAIOne = document.createElement('button');
const btnAITwo = document.createElement('button');
...
class Game {
    constructor() {
        ...
        this.bulletSize = 10;
        this.players = [
            new Player(canvas.width / 2 + grid * 4, canvas.height / 2, 30, 10, 'red', canvas.width * 0.75, 0),
            new Player(canvas.width / 2 - grid * 4, canvas.height / 2, 30, 10, 'blue', canvas.width * 0.25, 1),
        ];
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.code === 'Space') {
                this.bullets.push(new Bullet(this.players[0].x -
                    this.players[0].size -
                    this.bulletSize, this.players[0].y, this.bulletSize, this.bulletSpeed * -1, 'pink'));
            }
            if (e.code === 'KeyV') {
                this.bullets.push(new Bullet(this.players[1].x + this.players[1].size + 5, this.players[1].y, this.bulletSize, this.bulletSpeed, 'lightblue'));
            }
        });
    ...
    moveMind(mind, player) {
        let shootTime = Math.floor(Math.random() * 15);
        if (shootTime === 1) {
            this.bullets.push(new Bullet(player.index === 0
                ? player.x - player.size - this.bulletSize
                : player.x + player.size + this.bulletSize, player.y, this.bulletSize, player.index === 0 ? -this.bulletSpeed : this.bulletSpeed, 'lightblue'));
        }
        if (mind.count > 0) {
            mind.count--;
        }
...
class Player {
    constructor(x, y, size, speed, color, scorePos, index) {
        this.x = x;
        this.y = y;
        this.size = size;
        this.speed = speed;
...
class Mind {
    constructor(thinking, dirX, dirY, count) {
